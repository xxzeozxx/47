...
local XALConfig = Window.ConfigManager:CreateConfig("xal")
...
local BaseFolder = "WindUI/" .. (Window.Folder or "XAL") .. "/config/"
...
do
    ...
    _G.XAL_AutoAcceptTradeEnabled = false 
    ...
    if PromptController and PromptController.FirePrompt and Promise then
        ...
        PromptController.FirePrompt = function(self, promptText, ...)
            ...
            if _G.XAL_AutoAcceptTradeEnabled and type(promptText) == "string" and promptText:find("Accept") and promptText:find("from:") then
        ...
        warn("[XAL] Gagal memuat PromptController/Promise untuk Auto Accept Trade.")
    end
end
        ...
        if data then
            ...
            if targetPlayer.Character then
                for _, v in ipairs(targetPlayer.Character:GetChildren()) do
                    if v.Name == "XALESP" and v:IsA("BillboardGui") then pcall(function() v:Destroy() end) end
                end
            end
        end
    end
...
do
    ...
    task.spawn(function()
        ...
        if S1 and FishingController then
            ...
            FishingController.RequestChargeFishingRod = function(...)
                if _G.XAL_BlatantActive then return end 
                return Old_Charge(...)
            end
            FishingController.SendFishingRequestToServer = function(...)
                if _G.XAL_BlatantActive then return false, "Blocked by XAL" end
                return Old_Cast(...)
            end
        end
    end)
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        if _G.XAL_BlatantActive and not checkcaller() then
...
local function GetItemsToFavorite()
    ...
    local accept = trade:Toggle({
        ...
        Value = false,
        Callback = function(state)
            _G.XAL_AutoAcceptTradeEnabled = state
...
do
    ...
    local function CreateKaitunUI()
        local old = game.CoreGui:FindFirstChild("XALKaitunStats")
        if old then old:Destroy() end
        local sg = Instance.new("ScreenGui")
        sg.Name = "XALKaitunStats"
        ...
        Callback = function(state)
            ...
            if state then
                ...
                if not _G.BlackScreenGUI then
                    ...
                    _G.BlackScreenGUI.Name = "XAL_BlackBackground"
        ...
        Callback = function(state)
            ...
            if state then
                ...
                for _, gui in ipairs(PlayerGui:GetChildren()) do
                    if gui:IsA("ScreenGui") and gui.Name ~= "WindUI" and gui.Name ~= "CustomFloatingIcon_XAL" then
...
do
    ...
    local WEBHOOK_URL = ""
    local WEBHOOK_USERNAME = "XAL Notify" 
    ...
    local GLOBAL_WEBHOOK_USERNAME = "XAL | Community"
    ...
    local function onFishObtained(itemId, metadata, fullData)
        local success, results = pcall(function()
            ...
            if isWebhookEnabled and WEBHOOK_URL ~= "" and isUserFilterMatch then
                local title_private = string.format("<:TEXTURENOBG:1438662703722790992> XAL | Webhook\n\n<a:ChipiChapa:1438661193857503304> New Fish Caught! (%s)", fishName)
            ...
            if isGlobalTarget and GLOBAL_WEBHOOK_URL ~= "" then 
                ...
                local title_global = string.format("<:TEXTURENOBG:1438662703722790992> XAL | Global Tracker\n\n<a:globe:1438758633151266818> GLOBAL CATCH! %s", fishName)
                ...
                local globalEmbed = {
                    ...
                    thumbnail = { url = imageUrl },
                    footer = {
                        text = string.format("XAL Community| Player: %s | %s", censoredPlayerName, os.date("%Y-%m-%d %H:%M:%S"))
                    }
                }
        ...
        if not success then
            warn("[XAL Webhook] Error processing fish data:", results)
        end
    end
...
do
    ...
    ConfigSection:Button({
        ...
        Callback = function()
            ...
            if SelectedConfigName ~= "xal" then
                local success, err = pcall(function()
                    local mainContent = readfile(BaseFolder .. "xal.json")
    ...
    ConfigSection:Button({
        ...
        Callback = function()
            if SelectedConfigName == "" or SelectedConfigName == "xal" then 
            ...
            if isfile(path) then
                ...
                RefreshConfigList(ConfigDropdown)
                ConfigNameInput:Set("xal")
                SelectedConfigName = "xal"
            else
        ...
        Buttons = {
            {
                ...
                Icon = "link",
                Callback = function()
                    setclipboard("https://dsc.gg/xal")
                    WindUI:Notify({
                        Title = "Link Disalin!",
                        Content = "Link Discord XAL berhasil disalin.",
                        Duration = 3,
                        Icon = "copy",
                    })
                end,
...
local function CreateFloatingIcon()
    local existingGui = PlayerGui:FindFirstChild("CustomFloatingIcon_XAL")
    if existingGui then existingGui:Destroy() end

    local FloatingIconGui = Instance.new("ScreenGui")
    FloatingIconGui.Name = "CustomFloatingIcon_XAL"
...
WindUI:Notify({ Title = "XAL Was Loaded", Content = "Press [F] to open/close the menu", Duration = 5, Icon = "info" })
...
task.spawn(function()
    ...
    while true do
         task.wait(10)
         pcall(function() XALConfig:Save() end)
    end
end)
InitializeIcon()
